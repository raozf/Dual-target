...
def generate_rotation_z_matrix(theta):
    ...
    mat[1, 1] = math.cos(theta)
    return mat
def generate_random_rotation_matrix(angle1=0, angle2=45):
...
def generate_random_rotation_matrix1(angle1=0, angle2=5):
    thetax = np.random.uniform() * np.pi * angle2 / 180.0
    thetay = np.random.uniform() * np.pi * angle2 / 180.0
    thetaz = np.random.uniform() * np.pi * angle2 / 180.0
    matx = generate_rotation_x_matrix(thetax)
...
def generate_random_tranlation_vector2(range1=-0.5, range2=0.5):
    tranlation_vector = np.random.uniform(range1, range2, size=(3, )).astype(np.float32)
    return tranlation_vector

def generate_random_rotation_matrix2(angle1=0, angle2=45):
    thetax = np.random.uniform() * np.pi * angle2 / 180.0
    thetay = np.random.uniform() * np.pi * angle2 / 180.0
    thetaz = np.random.uniform() * np.pi * angle2 / 180.0
    matx = generate_rotation_x_matrix(thetax)
...
def generate_random_rotation_matrix3(angle1=0, angle2=180):
    thetax = np.random.uniform(0.50, 1.0) * np.pi * angle2 / 180.0
    thetay = np.random.uniform(0.50, 1.0) * np.pi * angle2 / 180.0
    thetaz = np.random.uniform(0.50, 1.0) * np.pi * angle2 / 180.0
    matx = generate_rotation_x_matrix(thetax)
    maty = generate_rotation_y_matrix(thetay)
    matz = generate_rotation_z_matrix(thetaz)
    return np.dot(matx, np.dot(maty, matz))

def generate_random_tranlation_vector1(range1=-0.1, range2=0.1):
    tranlation_vector = np.random.uniform(range1, range2, size=(3, )).astype(np.float32)
    return tranlation_vector
