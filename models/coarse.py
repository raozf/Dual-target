...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.emb_dims = 1216                      #if res: 1216  , else: 1024
        self.N = 1
        self.dropout = 0.0
        self.ff_dims = 1216
        self.n_heads = 4
...
def z_order_sorting(points):
    x, y, z = (points * 1000).astype(np.int64).T
    z_orders = np.zeros(points.shape[0], dtype=np.int64)
    for i in range(10):
        mask = 1 << i
        z_orders |= ((x & mask) << (2 + 2 * i)) | ((y & mask) << (1 + 2 * i)) | ((z & mask) << i)
    return np.argsort(z_orders)
...
#        reduced_points = pca.fit_transform(batch_cpu)
        sorted_indices = z_order_sorting(batch_cpu)
...
#        self.weights_a = nn.Parameter(torch.randn(1))
    ...
    def forward(self, x, pooling=True):
#        x = process_point_clouds(x.detach().permute(0, 2, 1)).permute(0, 2, 1).contiguous()
...
#        L1 = process_point_clouds(L.detach().permute(0, 2, 1))
        # if isinstance(L, torch.Tensor):
        #    print("L 是一个 PyTorch tensor")
        #    # 检查 L 是否有梯度
        #    if L.requires_grad:
        #         print("L 有梯度")
        #    else:
...
#        out_dims=[64, 64, 64, 128, 1024]
        ...
        self.decoder_ol = PointNet_de(in_dim=5888,               #if res: 6080  , else: 5120
                                   gn=gn,
                                   out_dims=[2048,2048,512,256,2])
#       [2048, 2048,512, 256, 2]
        self.decoder_qt = MLPs(in_dim=2944,                      #if res: 2432  , else: 2048
                               gn = gn,
