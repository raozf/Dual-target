...
def sample_and_group(xyz, points, M, radius, K, use_xyz=True, rt_density=False):
    ...
    if M < 0:
        ...
        new_xyz = gather_points(xyz, fps(xyz, M))                                        #M大于0，使用FPS（Farthest Point Sampling）算法从输入点云xyz中选择M个点作为新的中心点；如果M小于0，则不进行下采样，new_xyz就是输入的xyz 
    if rt_density:
        grouped_inds, density = ball_query(xyz, new_xyz, radius, K,
                                           rt_density=True)
    else:
        grouped_inds = ball_query(xyz, new_xyz, radius, K, rt_density=False)
    grouped_xyz = gather_points(xyz, grouped_inds)                                       #grouped_xyz, shape=(B, M, K, 3), new_xyz的邻居点
    
    grouped_xyz1 = grouped_xyz -  torch.unsqueeze(new_xyz, 2).repeat(1, 1, K, 1)         #对于每个中心点，计算其邻居点相对于它的偏移量
    squared_offset = torch.sum(grouped_xyz1 ** 2, dim=-1, keepdim=True)                  
    norms = torch.sqrt(squared_offset)                                                   #计算二范数

    combined = torch.cat((grouped_xyz1, norms), dim=-1)                                  # 连接偏移量和二范数

    if points is not None:                                                               # normal信息，计算PPF特征
        grouped_points = gather_points(points, grouped_inds)
        if use_xyz:
            new_points = torch.cat((combined.float(), grouped_points.float()), dim=-1)
        else:
            new_points = grouped_points
    else:
        new_points = grouped_xyz1
    if rt_density:
        return new_xyz, new_points, grouped_inds, grouped_xyz1, density
    return new_xyz, new_points, grouped_inds, grouped_xyz1, grouped_xyz
