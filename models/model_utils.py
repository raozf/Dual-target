...
def sample_and_group(xyz, points, M, radius, K, use_xyz=True, rt_density=False):
    ...
    grouped_xyz1 = grouped_xyz -  torch.unsqueeze(new_xyz, 2).repeat(1, 1, K, 1)
    squared_offset = torch.sum(grouped_xyz1 ** 2, dim=-1, keepdim=True)
    norms = torch.sqrt(squared_offset)

# 连接偏移量和二范数
    combined = torch.cat((grouped_xyz1, norms), dim=-1)
#    norms = torch.norm(grouped_xyz, p=2, dim=-1)  # 在最后一个维度上计算二范数
