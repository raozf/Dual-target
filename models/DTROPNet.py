...
from models import InitModule, AMPRModule, gather_points, weighted_icp


class DTROPNet(nn.Module):
    def __init__(self, args):
        super(DTROPNet, self).__init__()
        self.N1 = args.test_N1
        self.use_ppf = args.use_ppf
        self.Init= InitModule(in_dim=3, gn=True)
        self.AMPR = AMPRModule(args)
    ...
    def forward(self, src, tgt, num_iter=1, train=False):
        ...
        # CG module
        T0, x_ol, y_ol = self.Init(src,tgt)
        R, t = T0[:, :3, :3], T0[:, :3, 3]
