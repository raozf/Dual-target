...
#        self.Weight_Fused = WeightFused()
    def forward(self, feature, xyz, permute=False, use_ppf=False):
        ...
        if use_ppf:
            ...
            d_norm = torch.norm(grouped_xyz1, dim=-1)
            ppf_feat = torch.stack([nr_d, ni_d, nr_ni, d_norm], dim=-1) # (B, N, K, 3)
            new_points = torch.cat([new_points[..., :3], ppf_feat], dim=-1)
        xyz = torch.unsqueeze(xyz, dim=2).repeat(1, 1, self.K, 1)
        new_points = torch.cat([xyz, new_points[..., :3], ppf_feat], dim=-1)
...
#        print(x1.shape , x.shape)
        ...
        x = self.conv_fuse(x)
        return x
class AMPRModule(nn.Module):
    def __init__(self, args):
        super(AMPRModule, self).__init__()
