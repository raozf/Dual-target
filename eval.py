...
from models import Dual_target2, gather_points
from utils import npy2pcd, pcd2npy, inv_R_t, batch_transform, square_dists, \
    format_lines, vis_pcds,transform
...
def evaluate_ROPNet(args, test_loader):
    ...
    model.eval()
    dura = []
    r_mse, r_mae, t_mse, t_mae, r_isotropic, t_isotropic,ccd = [], [], [], [], [], [],[]
    src_recalls_op, src_precs_op = [], []
    src_recalls_rop, src_precs_rop = [], []
    with torch.no_grad():
        for i, (tgt_cloud,src_cloud,src_cloud_R,gtR,gtt) in tqdm(enumerate(test_loader)):
            if args.cuda:
            ...
            clip_chamfer_dist = dcp_metrics(src_cloud[:,:,:3], tgt_cloud[:,:,:3], gtR, gtt, R, t)
...
if __name__ == '__main__':
    ...
    dura, r_mse, r_mae, t_mse, t_mae, r_isotropic, t_isotropic = evaluate_ROPNet(args, test_loader)
    print_metrics('ROPNet改,三次迭代', dura, r_mse, r_mae, t_mse, t_mae,r_isotropic, t_isotropic)
